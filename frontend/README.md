# simple-chatbot 🚀
A minimal chatbot with a Django backend, React frontend, and Claude API integration (Need an Anthropic API key with a funded credit balance( Hope you have it own your own or like me, you can use a Friend's API key - No, i did not annoy him :) ) — built end-to-end, one concept at a time, as a learning project rather than scaffolded from a template.

[No live deployment — this is a local-only learning project, run via Docker Compose (see below).]
